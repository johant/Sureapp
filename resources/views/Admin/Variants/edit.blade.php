...
@section('header')
<h1 class="pull-left">
 Modificar Variante {{ $variant->name}}
 <small><i class="fa fa-arrow-circle-left"></i><a href="{{ route('variants.index') }}"> Regresar a la lista de Variantes</a></small>
</h1>
@stop
@section('content')
<div class="content">
<form id="frmTicket" class="form-horizontal"" action="{{ route('variants.update', $variant->id)}}" method="post" enctype="multipart/form-data" files="true" >
    {{ csrf_field() }}
                        ...
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <div class="form-group {{ $errors->has('name') ? 'has-error': ''}}">
                                    ...
                                    <div class="col-md-9">
                                        <div class="input-group input-group-required">
                                            <input class="form-control text-box single-line" id="name" name="name" type="text" value="{{ old('name', $variant->name)}}">
                                ...
                                <div class="form-group {{ $errors->has('description') ? 'has-error': ''}}">
                                    ...
                                    <div class="col-md-9">
                                        <div class="input-group input-group-required">
                                            <input class="form-control text-box single-line" id="description" name="description" type="text" value="{{ old('description', $variant->description)}}">
                                            <div class="input-group-btn">
                                                <span class="required">*</span>
                                            </div>
                                ...
                                <div class="form-group {{ $errors->has('brand_id') ? 'has-error': ''}}">
                                    <div class="col-md-3">
                                        <div class="label-wrapper">
                                            <label class="control-label" for="brand_id" title="">Marca</label>
                                        ...
                                        <div class="input-group input-group-required">
                                         <select class="form-control valid" name="brand_id" id="brand_id">
                                            ...
                                            @foreach($brands as $brand)
                                                <option value="{{ $brand->id }}" {{old('brand_id', $variant->brand_id ) == $brand->id ? 'selected': ''}}>{{ $brand->name }}</option>
                                            @endforeach
                                            </select>
                                            <div class="input-group-btn">
                                                <span class="required">*</span>
                                        ...
                                        <div class="input-group input-group-required">
                                         <select class="form-control valid" name="segmentation_id" id="segmentation_id">
                                            ...
                                            @foreach($segmentations as $segmentation)
                                                <option value="{{ $segmentation->id }}" {{old('segmentation_id', $variant->segmentation_id ) == $segmentation->id ? 'selected': ''}}>{{ $segmentation->name }}</option>
                                            @endforeach
                                            </select>
                                            <div class="input-group-btn">
                                                <span class="required">*</span>
