...
@section('header')
<h1 class="pull-left">
 Modificar Departamento {{ $city->name}}
 <small><i class="fa fa-arrow-circle-left"></i><a href="{{ route('cities.index') }}"> Regresar a la lista de Departamentos</a></small>
</h1>
@stop
@section('content')
<div class="content">
<form id="frmTicket" class="form-horizontal"" action="{{ route('cities.update', $city->id)}}" method="post" enctype="multipart/form-data" files="true" >
    {{ csrf_field() }}
                                ...
                                <div class="form-group {{ $errors->has('name') ? 'has-error': ''}}">
                                    ...
                                    <div class="col-md-9">
                                        <div class="input-group input-group-required">
                                            <input class="form-control text-box single-line" id="name" name="name" type="text" value="{{ old('name', $city->name)}}">
                                        ...
                                        <div class="input-group input-group-required">
                                         <select class="form-control valid" name="area_id" id="area_id">
                                            ...
                                            @foreach($areas as $area)
                                                <option value="{{ $area->id }}" {{old('area_id', $city->area_id ) == $area->id ? 'selected': ''}}>{{ $area->name }}</option>
                                            @endforeach
                                            </select>
                                            <div class="input-group-btn">
                                                <span class="required">*</span>
